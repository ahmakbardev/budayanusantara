<nav class="flex sticky top-0 bg-slate-100 z-50 text-black items-center justify-between px-5 py-2 w-full">
    <div class="flex-1">
        <img class="w-20 md:w-40" src="{{ asset('assets/images/logo/logo_transparent.png') }}" alt="">
    </div>
    <ul id="drawer" role="menu"
        class="sm:gap-3 flex-none max-md:hidden md:gap-8 transition-left ease-[cubic-bezier(0.4, 0.0, 0.2, 1)] delay-150  sm:flex  flex flex-col cursor-pointer absolute min-h-screen -left-48 sm:static w-48 top-0  sm:shadow-none shadow-xl sm:bg-transparent sm:flex-row sm:w-auto sm:min-h-0 ">
    ...
    <div class="flex gap-3 items-center justify-end flex-1">
        @guest
            <a href="{{ route('login') }}"
                class= " max-sm:hidden text-white bg-[#EA580C] hover:ring-[#EA580C] hover:bg-transparent hover:ring-1 hover:text-[#EA580C] transition-all ease-in-out py-2 px-5 rounded-md">Belajar
                Sekarang</a>
                ...
                <div class="drop-down hidden w-48 overflow-hidden bg-white rounded-md shadow absolute top-12 right-3">
                    <ul>
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            <li class="px-3 py-3 text-sm font-medium flex items-center space-x-2 hover:bg-slate-400">
                                @csrf
                                <button type="submit" class="w-full h-full flex items-center space-x-2">
        ...
        <script>
            ...
            document.addEventListener('click', (event) => {
                ...
                if (!div.contains(event.target) && !dropdown.contains(event.target)) {
        ...
        {{-- <div class="sm:hidden cursor-pointer" id="mobile-toggle">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path class="text-black" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
